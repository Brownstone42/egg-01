<template>
    <div class="public-tracking-page section">
        <div class="container">
            ...
            <div
                v-if="searchAttempted && !orderData && !isLoading && !error"
                class="notification is-warning mt-5"
            >
                <p>
                    No order found with the ID '{{ lastSearchedId }}'. Please check the ID and try
                    again.
                </p>
            </div>
        </div>
            ...
            try {
                ...
                if (orderSnap.exists()) {
                    ...
                    if (data.status === 'paid' || data.status === 'successful') {
                         // Transform Data to match OrderDetailDisplay props
                         const mappedOrder = {
                             id: orderSnap.id,
                             status: data.status,
                             ...
                             // Combine items into a pack string
                             pack: data.items 
                                 ? data.items.map(i => `${i.name} (${i.cycle})`).join(', ') 
                                 : (data.pack || '-'),
                             deliverySchedule: data.deliverySchedule || 'Every Monday', // Fallback
                             noteToDriver: data.noteToDriver || data.customer?.note || ''
                         }

                        this.orderData = mappedOrder
                    } else {
                        // Order exists but not paid
                         this.error = 'This order has not been paid yet or is still pending confirmation.'
                    }
                } else {
                    console.log('No such document!')
                    // Let the template handle "No order found" via !orderData check
                }
            } catch (err) {
